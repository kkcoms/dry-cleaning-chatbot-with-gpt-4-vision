...
export default function Sandbox() {
  ...
  const [showHeaders, setShowHeaders] = React.useState(true); // add this line to your component's state definitions
  ...
  const handleStarterMessageClick = (content) => {
    ...
    setMessageItems((prevItems) =>
      prevItems.filter((item) => item.role !== "starter")
    );
    setShowHeaders(false); // set showHeaders to false to hide the header
    // handleSubmit({ preventDefault: () => {} }); // uncomment if handleSubmit should be called
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const previous = messageItems
      .filter((item) => item.type === "text" && item.role !== "error")
      .map((item) => {
        return {
          role: item.role,
    ...
    try {
      const response = await fetch("/api/", {
        ...
        body: JSON.stringify({
          ...
          image:
            newUserItem.image &&
            Array.isArray(newUserItem.image) &&
            newUserItem.image.length > 0
              ? newUserItem.image
              : [],
        }),
      });
      ...
      if (
        ret.result.image &&
        Array.isArray(ret.result.image) &&
        ret.result.image.length > 0
      ) {
          ...
          {messageItems.map((item) => {
            if (item.role === "starter") {
              return (
                <div
                  key={item.id}
                  className={classes.starterMessage}
                  onClick={() => handleStarterMessageClick(item.content)}
                >
            ...
            return (
              <div key={item.id} className={classes.messageItem}>
                ...
                >
                  ...
                  {item.role === "error" && (
                    <p className={`${classes.text} ${classes.error}`}>
                      {item.content}
                    </p>
                  )}
                  {item.role !== "error" && (
                  ...
                  <div
                    className={
                      item.role !== "user" ? classes.close2 : classes.close
                    }
                  >
                    <CustomTheme>
                      <IconButton
                        disabled={isProcessing}
                        onClick={() => handleDeleteMessage(item.gid)}
                      >
                        <CloseIcon
          ...
          <CustomTheme>
            <Box component="form" onSubmit={handleSubmit} noValidate>
              <TextField
                ...
                placeholder={setCaption(
                  messageItems.length > 1 ? "send_reply" : "send_message"
                )}
                disabled={isProcessing}
                fullWidth
                inputRef={inputRef}
                  ...
                  startAdornment: (
                    <InputAdornment position="start">
                      <IconButton
                        disabled={
                          isProcessing ||
                          previewImage.length >= process.env.maxFileUploadCount
                        }
                        onClick={handleImage}
                      >
                        <ImageIcon />
